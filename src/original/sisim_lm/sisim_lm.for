...
c
      ...
      if(str(1:1).eq.' ') str(1:20) = 'sisim_lm.par        '
...
c
 ...
 44         read(lin,*,end=66,err=98)(var(j),j=1,nvari)
            if(var(ivrl).lt.tmin.or.var(ivrl).ge.tmax)go to 44
            MAXDAT = MAXDAT + 1
            go to 44
 66         continue
c
...
c
 ...
 33         read(lin,*,end=55,err=97) (var(j),j=1,nvari)
            if(var(itabvr).lt.tmin.or.var(itabvr).ge.tmax)go to 33
            MAXTAB = MAXTAB + 1
            go to 33
 55         continue
c
...
c
c Estimate the local distribution by indicator kriging. Missing
c values in the prior indicator mean file are identified to the 
c global cdf for the corresponding thresholds. Here missing
c values are considered to lie in ]0,1[.
c
                        do ic=1,ncut
                              if(primn(ic,index).lt.EPSLON.or.
     +                           primn(ic,index).gt.1.0) then
                                      primn(ic,index) = cdf(ic)
                              end if
...
c
                  ...
                  z1     = cnodez(index)
c
c Is this node informed by a hard datum?
c
